...
function findElementGreaterThan(arr, num) {
  ...
  let element = arr.find(value => value > num);
  if (element) {
    return element;
  } else {
...
function findIndexByProperty(arr, name) {
  ...
  return foundIndex >= 0 ? foundIndex : "Обьекта с таким именем не найдено"
}
...
function findLastEvenNumber(arr) {
  ...
  const parnenum= arr.findLast((element) => element % 2 === 0);
  return parnenum ? parnenum : "Парних чисел не знайдено";
...
function getPositiveArrayValues(arr) {
  ...
  const result = arrvalues.filter((num) => num > 0 );
  return result;
...
function removeAndDouble(arr, startIdx) {
  ...
  arr.splice(startIdx);
  const mnog = arr.map(element => element*2);
  return mnog;
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  const arrmap = arrflat.map(element => element*element);
...
function getLastElementsSumAndPrint(arr, n) {
  ...
  const sum = newarr.reduceRight((previousValue, currentValue) =>
    {
...
function findLastNegativeNumberIndex(arr) {
  ...
  const last = arr.findLastIndex(negativeNum);
  return last;
